...
//.mjs
import AvisModel from '../model/AvisModel.mjs';
import LigneModel from '../model/LigneModel.mjs';
...
const avisDAO = {
     ...
     * */
    addAvis: async (nouvelAvis, nomligne) => {
        // Vérifier les paramètres
        ...
        if (ligneVoted != null) {
            ...
            if (lastvoteoftheUser != null) {
                const msdiff = (new Date(nouvelAvis.date).getTime() - new Date(lastvoteoftheUser.date).getTime());
                if ((msdiff / (1000 * 60 * 60)) < 1) {
                    throw new Error("l'utilisateur à déjà voter sur cette ligne il y a moins d'1h");
                }
            }
        }
        else {
        ...
        // ajouter l'avis
        const result = await MongoAvis.findOneAndUpdate({ nomLigne: nomligne }, { $push: { avis: nouvelAvis } }, { new: true });
        //get la nouvelle ligne
        const newligne = await avisDAO.findByLigne(nomligne);
        if (newligne == null) {
            throw new Error("la ligne n'a pas été crée");
        }
        return newligne;
    },
    /**
    ...
    addLigne: async (ligne) => {
        ...
        if (await avisDAO.findByLigne(ligne.nomLigne) != null) {
            throw new Error("la ligne existe déjà");
        }
