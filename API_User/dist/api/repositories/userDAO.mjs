...
import crypter from '../tools/cryptage.mjs';
...
const userDAO = {
    ...
    findByEmailAndPassword: async (email, password) => {
        ...
        if (user.password === null)
            return null; // Si l'utilisateur n'a pas de mot de passe
        // Vérifier le mot de passe
        const validPassword = await bcrypt.compare(password, user.password);
        if (!validPassword)
            return null; // Si le mot de passe n'est pas valide
     ...
     * @returns {Promise<boolean>}
     */
    removeUser: async (email, password) => {
        // Vérifier si l'utilisateur existe
        if (email === null || password === null)
     ...
     */
    update: async (email, password, user) => {
        // Vérifier les paramètres
        if (!(user instanceof UserModel))
            throw new Error("Not a valid user");
        if (user.password === null || email === null || password === null)
            throw new Error("Parametre is null");
        if (!(typeof email === 'string') || !(typeof password === 'string'))
            throw new Error("Email or password is not a string");
        // Vérifier si l'utilisateur existe
